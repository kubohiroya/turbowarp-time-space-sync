...
describe('TimeSpaceSyncExtension', () => {
  ...
  it('uses localizable extension and block text', () => {
    const info = new TimeSpaceSyncExtension().getInfo() as {name: string; blocks: Array<{text: string}>};
  ...
  it('publishes documentation and a self-contained SVG block icon', () => {
    const info = new TimeSpaceSyncExtension().getInfo() as {docsURI: string; blockIconURI: string};
