...
function checkPolicy() {
  ...
  if (policy.productName !== 'TurboWarp-Time-Space-Sync') {
...
function checkPackageMetadata() {
  ...
  if (packageMetadata.repository?.url !== 'git+https://github.com/kubohiroya/turbowarp-time-space-sync.git') {
    errors.push('package.json repository.url must point to the current repository');
  }
  if (packageMetadata.bugs?.url !== 'https://github.com/kubohiroya/turbowarp-time-space-sync/issues') {
...
function checkReadmes() {
  ...
  if (!readmeJa.startsWith('# TurboWarp-Time-Space-Sync\n')) {
...
async function checkPackContents() {
  ...
  if (!files.has('dist/time-space-sync.js')) {
